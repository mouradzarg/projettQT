...
query.prepare("INSERT INTO courrier( id_courrier,nom_prenom_expediteur,nom_prenom_destinataire,statut_courrier,adresseDestinatire) "
                    "VALUES (  :id_courrier, :nom_prenom_expediteur, :nom_prenom_destinataire, :statut_courrier, :adresseDestinatire)");
...
bool courrier::modifier(QString statut_courrier,QString nom_prenom_expediteur,QString nom_prenom_destinataire,QString adresseDestinatire ,int id_courrier)
{
   QSqlQuery query;
   QString res= QString::number(id_courrier);
   query.prepare("UPDATE coli SET NOM_PRENOM_EXPEDITEUR = :c, NOM_PRENOM_DESTINATAIRE = :a, STATUT_COURRIER = :b, ADRESSEDESTINATIRE = :d where ID_COURRIER= :ID_COURRIER");
   query.bindValue(":id_courrier", res);
